...
namespace Probability
{
    ...
    static class Extensions
    {
        ...
        public static string DiscreteHistogram<T>(this IEnumerable<T> d)
        {
            ...
            int labelMax = dict.Keys.Select(x => x.ToString().Length).Max();
        ...
        public static string Separated<T>(this IEnumerable<T> items, string s) => string.Join(s, items);

        public static string Concatenated<T>(this IEnumerable<T> items) => string.Join("", items);

        public static string CommaSeparated<T>(this IEnumerable<T> items) => items.Separated(",");

        public static string NewlineSeparated<T>(this IEnumerable<T> items) => items.Separated("\n");

        public static string SpaceSeparated<T>(this IEnumerable<T> items) => items.Separated(" ");

        public static int Product(this IEnumerable<int> numbers) => numbers.Aggregate(1, (a, b) => a * b);

        public static int GCD(int a, int b) => b == 0 ? a : GCD(b, a % b);

        public static long GCD(long a, long b) => b == 0L ? a : GCD(b, a % b);
        ...
        public static long LCM(this IEnumerable<long> numbers) => numbers.Aggregate(1L, LCM);
