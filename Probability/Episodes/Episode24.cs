...
namespace Probability
{
    ...
    {
        static IDiscreteDistribution<bool> Flip() => Bernoulli.Distribution(1, 1).Select(x => x == 0);
        ...
        public static void DoIt()
        {
            ...
            var initial = DiscreteSingleton<int>.Distribution(n);

            IDistribution<int> Transition(int x) => (x == 0 || x == 2 * n) ? Empty<int>.Distribution : Flip().Select(b => b ? x - 1 : x + 1);
